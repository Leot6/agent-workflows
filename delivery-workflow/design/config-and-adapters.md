...
commit.trailers=Change-Id                  # generated by the vendored commit-msg hook
...
follows the project's build/lint/test/acceptance and commit conventions verbatim — including
for its **own** source commits (subjects verbatim, Change-Id via the vendored hook). The
...
- **One commit convention for both** (`commit.*` is the project's, not the repo's): a doc
  ...
  needs the same `commit-msg` hook wired.
